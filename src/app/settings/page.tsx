...
const integrations = [
  { name: "HubSpot", desc: "Sync contacts & deals", connected: true, color: "#FF7A59" },
  { name: "Slack", desc: "Deal notifications", connected: true, color: "#4A154B" },
  { name: "Zapier", desc: "Workflow automation", connected: false, color: "#FF4A00" },
  { name: "Supabase", desc: "Database & auth", connected: true, color: "#3ECF8E" },
  { name: "Stripe", desc: "Payment processing", connected: false, color: "#635BFF" },
  { name: "Google Workspace", desc: "Email & calendar sync", connected: true, color: "#4285F4" },
  { name: "CompanyCam", desc: "Job photos", connected: true, color: "#00A3E0" },
  { name: "PandaDoc", desc: "E-signatures", connected: true, color: "#4BBF6B" },
];
            ...
            <div className="card space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <span className="badge bg-[#F0A500]/20 text-[#0B1F3A] font-bold">Enterprise</span>
            ...
            <div className="card space-y-4">
              ...
              {[
                ...
                <div key={u.email} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  ...
                  <span className="badge bg-[#0B1F3A]/10 text-[#0B1F3A]">{u.role}</span>
            ...
            <div className="card space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {integrations.map((int) => (
                  <div key={int.name} className="flex items-center gap-3 p-4 rounded-lg border border-[#E5E7EB] hover:border-[#F0A500]/50 transition-colors">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: int.color + "20" }}>
                      <Zap size={18} style={{ color: int.color }} />
                    </div>
                    <div className="flex-1">
                      <p className="font-medium text-[#0B1F3A]">{int.name}</p>
                      <p className="text-xs text-[#9CA3AF]">{int.desc}</p>
                    </div>
                    <button className={`text-sm px-3 py-1 rounded-lg font-medium ${int.connected ? "bg-green-100 text-green-700" : "bg-gray-100 text-[#0B1F3A]"}`}>
              ...
              {["New deal created", "Deal stage changed", "Task assigned to you", "Ticket assigned to you", "Contact created", "Weekly digest report"].map(pref => (
                <div key={pref} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50">
                  ...
                  <div className="flex gap-4">
                    <label className="flex items-center gap-1.5 text-sm text-[#9CA3AF]">
